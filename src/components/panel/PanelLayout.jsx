...
export const PanelLayout = component$(({ onLogout$ }) => {
  const activeTab = useSignal('products'); // Default olarak products tab'ını aç
...
const DashboardContent = component$(() => {
  ...
  useTask$(async () => {
    // SSR/Client farkı gözetmeden fetch ile oku
    try {
      const response = await fetch('/db/products.json');
      if (response.ok) {
        const data = await response.json();
        stats.totalProducts = Array.isArray(data.products) ? data.products.length : 0;
        stats.totalBrands = Array.isArray(data.brands) ? data.brands.length : 0;
        stats.totalContacts = Array.isArray(data.contacts) ? data.contacts.length : 0;
        stats.loaded = true;
      }
    } catch (err) {
      // Hata durumunda 0 bırak
      stats.loaded = true;
    }
  });
